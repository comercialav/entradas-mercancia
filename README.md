...
2. Copia el archivo `.env.local` de ejemplo (si no existe) y define tus credenciales de Firebase + endpoint de notificaciones:
   ...
   ```
   PROOVEDORES_AV_FIREBASE_API_KEY=...
   PROOVEDORES_AV_FIREBASE_AUTH_DOMAIN=...
   PROOVEDORES_AV_FIREBASE_PROJECT_ID=...
   PROOVEDORES_AV_FIREBASE_STORAGE_BUCKET=...
   PROOVEDORES_AV_FIREBASE_MESSAGING_SENDER_ID=...
   PROOVEDORES_AV_FIREBASE_APP_ID=...
   PROOVEDORES_AV_NOTIFY_ENDPOINT=https://<tu-region>-<tu-proyecto>.cloudfunctions.net/notify
   PROOVEDORES_AV_DISABLE_NOTIFICATIONS=true   # opcional para pausar los correos en desarrollo
   ```
...
5. Copia la URL resultante y colócala en `PROOVEDORES_AV_NOTIFY_ENDPOINT` para que el frontend pueda disparar los correos.
